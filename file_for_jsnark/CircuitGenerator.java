...
public abstract class CircuitGenerator {
	...
	public void writeCircuitFilePaillier() {
		try {
			PrintWriter printWriter = new PrintWriter(new BufferedWriter(new FileWriter(getName() + "WithoutEval" + ".arith")));
	...
	public void getCircuitFile() {
		try {
			BufferedReader reader;
			reader = new BufferedReader(new FileReader(
					"Horner_polynomial_eval_big_int_paillier_gadget.arith"));
	...
	public void writeAllPailliersCoefsWithN(ArrayList<BigInteger> coeff_in_paillier, BigInteger x, Paillier_PublicKey publicKey) {
		try {
			PrintWriter printWriter = new PrintWriter(new BufferedWriter(new FileWriter("AllPailliersCoeffAndInformation.out")));

			printWriter.println(publicKey.getnSquared());
			printWriter.println(x);
			for (BigInteger i : coeff_in_paillier) {
	...
	public void genInputEval(){
		circuitEvaluator = new CircuitEvaluator(this);
		generateSampleInput(circuitEvaluator);
	}

	public void evalCircuitWithoutInput(){
		circuitEvaluator.evaluate();
	}
	...
	public void prepFilesPaillier() {
		writeCircuitFilePaillier();
		if (circuitEvaluator == null) {
			throw new NullPointerException("evalCircuit() must be called before prepFiles()");
		}
		circuitEvaluator.writeInputFilePaillier();
	}

	public void circuitFromFile() {
		getCircuitFile();
		if (circuitEvaluator == null) {
			throw new NullPointerException("evalCircuit() must be called before prepFiles()");
		}
