...
public:
    ...
    size_t get_g1_window(){
        return this->g1_window;
    }
    window_table<G1<ppT> > get_g1_table(){
        return this->g1_table;
    }
    size_t get_g2_window(){
        return this->g2_window;
    }
    window_table<G2<ppT> > get_g2_table(){
        return this->g2_table;
    }

    void set_At_save(Fr<ppT> At_save){
        this->At_save = At_save;
    }

    void set_Bt_save(Fr<ppT> Bt_save){
        this->Bt_save = Bt_save;
    }

    void set_Kt_save(Fr<ppT> Kt_save){
        this->Kt_save = Kt_save;
    }
};

template<typename ppT>
class return_container_for_update_generator {
private: 
    Fr<ppT> FFT_evaluation_point;
    random_container_key<ppT> random_container;
    r1cs_ppzksnark_keypair<ppT> key_pair;
public:
    return_container_for_update_generator(Fr<ppT> FFT_evaluation_point,
        random_container_key<ppT> random_container,
        r1cs_ppzksnark_keypair<ppT> key_pair):
...
template <typename ppT>
return_container_for_update_generator<ppT> r1cs_ppzksnark_generator_for_update(const r1cs_ppzksnark_constraint_system<ppT> &cs)
{
    ...
    pk.print_size();
    vk.print_size();
    return_container_for_update_generator<ppT> return_container(t, container_random, r1cs_ppzksnark_keypair<ppT>(std::move(pk), std::move(vk)));
                    ...
                    random_container_key<ppT> random_container)
{
    ...
    const  libff::Fr<ppT> alphaA = random_container.get_alphaA(),
        alphaB = random_container.get_alphaB(),
        alphaC = random_container.get_alphaC(),
        rA = random_container.get_rA(),
        rB = random_container.get_rB(),
        beta = random_container.get_beta(),
        gamma = random_container.get_gamma();
    const libff::Fr<ppT> rC = rA * rB;
