...
template<typename FieldT, typename ppT>
FieldT R1CS_Polynomial_factory<FieldT, ppT>::r1cs_to_qap_instance_map_with_evaluation_point(const FieldT &FFT_evaluation_point, FieldT QAPPoly, 
        FieldT new_polynomial_coefficient, FieldT last_polynomial_coefficient, uint64_t index) {
    libff::enter_block("Call to r1cs_to_qap_instance_map_with_evaluation_point");
    ...
    QAPPoly -= u * last_polynomial_coefficient;
    QAPPoly += u * new_polynomial_coefficient; 
    libff::leave_block("Call to r1cs_to_qap_instance_map_with_evaluation_point");
    return QAPPoly;

}

template<typename FieldT, typename ppT>
r1cs_ppzksnark_keypair<ppT> R1CS_Polynomial_factory<FieldT, ppT>::update_proving_key_compilation(uint64_t coef_index, libff::Fr<ppT> FFT_evaluation_point) {
    ...
    if(this->insert_error_for_test != 2) {
        ...
        random_container.At_save = r1cs_to_qap_instance_map_with_evaluation_point(FFT_evaluation_point, 
    ...
    if(!this->already_changed_one_time) {
        ...
        (*this->current_key_pair).pk.A_query.values.emplace((*this->current_key_pair).pk.A_query.values.begin(), knowledge_commitment<libff::G1<ppT>, 
                libff::G1<ppT>>(random_container.At_save * random_container.rA * libff::G1<ppT>::one(),
        ...
        random_container.Bt_save = r1cs_to_qap_instance_map_with_evaluation_point(FFT_evaluation_point, 
        ...
        random_container.Kt_save = random_container.Kt_save - At_save * random_container.rA * 
                ...
                random_container.beta - Bt_save * random_container.rB * random_container.beta + 
...
return_container_key_generator_for_update<ppT> R1CS_Polynomial_factory<FieldT, ppT>::r1cs_ppzksnark_key_generator_for_update() {
    ...
    const  libff::Fr<ppT> alphaA = libff::Fr<ppT>::random_element(), alphaB = libff::Fr<ppT>::random_element(), 
            alphaC = libff::Fr<ppT>::random_element(), rA = libff::Fr<ppT>::random_element(),
            rB = libff::Fr<ppT>::random_element(), beta = libff::Fr<ppT>::random_element(),
    ...
    r1cs_ppzksnark_verification_key<ppT> vk = r1cs_ppzksnark_verification_key<ppT>(alphaA_g2, alphaB_g1, alphaC_g2,
            gamma_g2, gamma_beta_g1, gamma_beta_g2, rC_Z_g2, encoded_IC_query);
    r1cs_ppzksnark_proving_key<ppT> pk = r1cs_ppzksnark_proving_key<ppT>(std::move(A_query), std::move(B_query),
            std::move(C_query), std::move(H_query), std::move(K_query), std::move(cs_copy));

    random_container_key<ppT> container_random = random_container_key<ppT>(alphaA, alphaB, alphaC, rA, rB, beta,
            gamma, At_save, Bt_save, Kt[2]);
